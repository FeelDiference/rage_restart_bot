...
class ServerMonitor:
    ...
    def check_server_health(self) -> Tuple[ServerHealth, Dict[str, Any]]:
        ...
        available_ports = sum(1 for result in port_results.values() if result['available'])
    ...
    def _determine_health_status(self, available_ports: int, total_ports: int, 
                               http_healthy: bool) -> ServerHealth:
        """
        ...
        main_port_available = any(port == 30120 for port in self.tcp_ports 
    ...
    def format_health_report(self, health: ServerHealth, details: Dict[str, Any]) -> str:
        """
        ...
        if server_data and server_data.get('success'):
            ...
            report += f"⏱️ **Uptime:** {server_info.get('uptime_formatted', 'Неизвестно')}\n"
        ...
        for port, info in details['ports'].items():
            ...
            response_time = f" ({info['response_time_ms']}ms)" if info['response_time_ms'] else ""
        ...
        http_time = f" ({http_info['response_time_ms']}ms)" if http_info['response_time_ms'] else ""
